...
class App extends React.Component<any, any> {
  ...
  updateWidgetHtml = async (id: number) => {
  ...
  saveActiveWidget = async (event: any) => {
    ...
    await this.updateWidgetHtml(this.state.activeItem);
    this.closeModal();
  };

  isGridConflict = (x: number, y: number, w: number, h: number) => {
    const ulc = {x, y};
  ...
  addWidget = async (type: any) => {
    ...
    await this.props.api.saveLayout(this.props.dashboardId, newLayout);
    await this.showEditForm(id);
  };
  ...
  generateDOM = () => (
    this.state.widgets.map((widget: any) => (
      <div key={widget.config.i}
           className={`ld-widget-container ${this.props.readOnly || widget.config.static ? '' : 'ld-widget-container--editable'}`}>
        <Widget key={widget.config.i} widget={widget} readOnly={this.props.readOnly || widget.config.static}
                onEditClick={this.onEditClick(widget.config.i)}/>
      </div>
    ))
  );
  ...
  renderModal = () => (
    ...
    >
      <div className="modal-header">
        ...
        <button className="close" onClick={this.closeModal}><span aria-hidden="true" className="hidden">Close</span>
        </button>
      </div>
      <div className="modal-body">
        ...
        {this.state.loadingEditHtml
          ? <span className="ld-modal__loading">Loading...</span>
          : <div ref={this.formRef} dangerouslySetInnerHTML={{__html: this.state.editHtml}}/>}
      </div>
